...
public:
  ...
  int Load(int _depth, std::string _directory_path, std::string _name_header)
  {
    ...
    for (int i = 0; i < depth; i++)
    {
      // std::string file_full_path = _directory_path + _name_header + "_1_8" + std::to_string(rate) + ".map";
      std::string file_full_path = "/home/ubuntu/catkin_ws2/src/Autonomous_Mobile_Robot-master/map/simulation/grid/grid_1_1.map";
