...
class CeresGridPoseOptimizer
{
  ...
  {
    TranslationCostFunctor(const double _x, const double _y, const double _factor) : x(_x), y(_y), factor(_factor){};
...
public:
                  ...
                  double _scan_factor = 1.0, double _translation_factor = 200.0, double _theta_factor = 80.0)
  {
    ...
    _result.x = pose[0];
    _result.y = pose[1];
    _result.theta = pose[2];
    while (_result.theta >= M_PI)
      _result.theta -= M_PI * 2.0;
    while (_result.theta < -M_PI)
      _result.theta += M_PI * 2.0;
