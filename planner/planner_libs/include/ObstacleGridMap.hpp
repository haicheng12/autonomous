...
public:
  ...
  ObstacleGridMap(const double _cell_size = 0.05, const double _x_min = -20.0, const double _x_max = 20.0,
                  const double _y_min = -20.0, const double _y_max = 20.0) : BaseGridMap(INIT_VAL, _cell_size, _x_min, _x_max, _y_min, _y_max){};
  ...
  void InputOccupancyGrid(const nav_msgs::OccupancyGrid &_ocp_grid)
  {
    ...
    x_min = origin_x;
    x_max = origin_x + tmp_w;
    w = tmp_w;
    y_min = origin_y;
    y_max = origin_y + tmp_h;
    h = tmp_h;
    idx_w = _ocp_grid.info.width;
    idx_h = _ocp_grid.info.height;
