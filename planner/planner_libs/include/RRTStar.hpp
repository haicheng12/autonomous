...
protected:
  struct Node
  {
    ...
    Node(const double _cost, const geometry_msgs::Pose2D &_pose, const Ptr &_prev = nullptr) : prev(_prev), cost(_cost), pose(_pose){};
  ...
  void PathCleanup(const std::vector<geometry_msgs::Pose2D> &_path, std::vector<geometry_msgs::Pose2D> &_result)
  { // 至近距離のノードを省略する
    _result.clear();
  ...
  bool CheckCollisionForEdge(const geometry_msgs::Pose2D &_st_pose, const geometry_msgs::Pose2D &_end_pose)
  {
    ...
    int x_add, y_add;
    if (idx_st_x < idx_end_x)
      x_add = 1;
    else
      x_add = -1;
    if (idx_st_y < idx_end_y)
      y_add = 1;
    else
      y_add = -1;
...
public:
  ...
  bool Generate(const geometry_msgs::Pose2D &_start, const geometry_msgs::Pose2D &_goal, std::vector<geometry_msgs::Pose2D> &_result)
  {
    ...
    if (!CheckCollisionForEdge(_start, _goal))
    { // 例外 直線でコリジョンないならそれを返却
                                ...
                                visualization_msgs::Marker &_edges, visualization_msgs::Marker &_result)
  {
    ...
    for (int i = 0; i < node_list.list.size(); i++)
    {
      ...
      p.x = node_list.list[i]->pose.x;
      p.y = node_list.list[i]->pose.y;
      p.z = 0.0;
      _nodes.points.emplace_back(p);
      if (node_list.list[i]->prev != nullptr)
      {
        geometry_msgs::Point p2;
        p2.x = node_list.list[i]->prev->pose.x;
        p2.y = node_list.list[i]->prev->pose.y;
        p2.z = 0.0;
