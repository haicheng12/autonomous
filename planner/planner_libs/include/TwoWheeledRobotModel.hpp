...
public:
                       ...
                       const double _max_omega = 2.5, const double _min_omega = -2.5)
  {
    ...
    init_twist.linear.z = 0.0;
    init_twist.angular.x = 0.0;
    init_twist.angular.y = 0.0;
    init_twist.angular.z = 0.0;
    SetStateTwist(init_twist);
  };
  ~TwoWheeledRobotModel(){};

  inline void SetAccelerationLimit(const double _max, const double _min)
  {
    max_acceleration = _max;
    min_acceleration = _min;
  };
  inline void GetAccelerationLimit(double &_max, double &_min)
  {
    _max = max_acceleration;
    _min = min_acceleration;
  };
  inline void SetAngAccelerationLimit(const double _max, const double _min)
  {
    max_ang_acceleration = _max;
    min_ang_acceleration = _min;
  };
  inline void GetAngAccelerationLimit(double &_max, double &_min)
  {
    _max = max_ang_acceleration;
    _min = min_ang_acceleration;
  };
  inline void SetVelocityLimit(const double _max, const double _min)
  {
    max_velocity = _max;
    min_velocity = _min;
  };
  inline void GetVelocityLimit(double &_max, double &_min)
  {
    _max = max_velocity;
    _min = min_velocity;
  };
  inline void SetOmegaLimit(const double _max, const double _min)
  {
    max_omega = _max;
    min_omega = _min;
  };
  inline void GetOmegaLimit(double &_max, double &_min)
  {
    _max = max_omega;
    _min = min_omega;
  };
  ...
  inline std::vector<geometry_msgs::Pose2D> GeneratePredictionPath(const double _time, const double _dt) const
  {
    ...
    while (t < _time)
    {
      ...
      while (next_pose.theta <= -M_PI)
        next_pose.theta += 2.0 * M_PI;
      while (next_pose.theta > M_PI)
        next_pose.theta -= 2.0 * M_PI;
