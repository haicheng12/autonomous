...
protected:
  ...
  {
    int idx_radius;
    for (idx_radius = 0; idx_radius < (idx_w / 2); idx_radius++)
    { // 一定値以下になる半径を算出
...
public:
  ...
  PotentialGridMap(const double _cell_size = 0.05, const double _x_min = -20.0, const double _x_max = 20.0,
                   ...
                   const double _gain_kg = 1.0, const double _gain_ko = 1.0, const double _gain_radius = 0.1) : BaseGridMap(INIT_VAL, _cell_size, _x_min, _x_max, _y_min, _y_max)
  {
