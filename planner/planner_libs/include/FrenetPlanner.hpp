...
class FrenetPlanner
{
  ...
  void CalcCartesianPath(int _st_g_path_idx, FrenetPath &_frenet_path) const
  {
    ...
    for (int i = 0; i < _frenet_path.d.size(); i++)
    { // xy座標に変換
  ...
  bool CheckInversionNode(const int _st_g_path_idx, const std::vector<geometry_msgs::Pose2D> &_path)
  { // 折り返すような挙動が無いかチェック
  ...
  inline bool CheckLeft(const geometry_msgs::Pose2D &_pose,
                        const geometry_msgs::Pose2D &_st, const geometry_msgs::Pose2D &_en) const
  { // 線分の左右チェック
  ...
  inline void UpdateFrenetPath(const int _frenet_min_idx, FrenetPath &_frenet_path) const
  { // 通り過ぎた分を削除
    if (_frenet_min_idx > 0 && _frenet_min_idx <= _frenet_path.path.size() - 1)
    { // 空にはしない
  ...
  inline void RemoveCollisionSection(FrenetPath &_frenet_path) const
  { // 空にはしない
...
public:
  ...
  GenerateResult Generate(const geometry_msgs::Pose2D _pose, std::vector<geometry_msgs::Pose2D> &_path)
  {
    ...
    if (cum_dist.back() < min_pred_dist)
    { // グローバルパスの長さがmin_pred_distより小さい場合
      _path = g_path;
    ...
    if (traveled_distance < (generated_frenet_path->path.size() / regenerate_timing) &&
            generated_frenet_path->collision_idx == -1 ||
        generated_frenet_path->path.size() < REGENERATE_LIMIT_LENGTH)
    {
  ...
  void GenerateVisualizationMsg(const std_msgs::Header &_header, visualization_msgs::Marker &_vis_path)
  {
    ...
    for (int i = 0; i < frenet_paths.size(); i++)
    {
      ...
      for (int j = 0; j < frenet_path->path.size() - 1; j++)
      {
        ...
        p1.x = frenet_path->path[j].x;
        p1.y = frenet_path->path[j].y;
        p1.z = 0.0;
        p2.x = frenet_path->path[j + 1].x;
        p2.y = frenet_path->path[j + 1].y;
        p2.z = 0.0;
